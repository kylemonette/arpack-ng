...
c  application time by mydsapps (a fork of dsapps that accepts it as a
c  new input) instead of calling dsapps directly with a shift list.
c
c\Usage:
c  call mydsaup2
...
c
c     %--------------------------------------------------------------%
c     | Local workspace for the full eigendecomposition of the       |
c     | current KEV+NP tridiagonal H. Computed ONCE per iteration    |
...
c     | time by mydsapps. ARROWEIGVAL/ARROWEIGVEC carry data from    |
c     | the computation site to the shift site, across the possible  |
c     | ISHIFT=0 reverse-communication exit in between, so they are  |
c     | SAVEd allocatables (re-sized if LDH ever changes between     |
c     | problems). ARROWSUBD/ARROWWORK are scratch used only at the  |
c     | computation site itself.                                     |
c     %--------------------------------------------------------------%
c
      integer    arrowierr
      Double precision
...
c
         if (allocated(arroweigval)) then
            if (size(arroweigvec,1) .ne. ldh) then
               deallocate (arroweigval, arroweigvec)
            end if
         end if
         if (.not. allocated(arroweigval)) then
...
c
         call dcopy (kplusp, arroweigval, 1, ritz, 1)
         do 45 j = 1, kplusp
            bounds(j) = rnorm*abs(arroweigvec(kplusp,j))
   45    continue
c
c        %----------------------------------------------------%
...
c
c        %-----------------------------------------------------------%
c        | Select the NEV "desired" eigenpairs directly: sort the    |
c        | fresh eigenvalues ARROWEIGVAL by the user's WHICH         |
c        | preference (largest algebraic, largest magnitude, etc.),  |
c        | carrying an index array along as the permutation.         |
